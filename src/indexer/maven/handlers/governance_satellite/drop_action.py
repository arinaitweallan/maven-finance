...
async def drop_action(
    ctx: HandlerContext,
    drop_action: TezosTransaction[DropActionParameter, GovernanceSatelliteStorage],
) -> None:

    try:
        # Get operation info
        governance_satellite_address    = drop_action.data.target_address
        action_storage                  = drop_action.storage.governanceSatelliteActionLedger[drop_action.parameter.root]
        action_id                       = int(drop_action.parameter.root)
