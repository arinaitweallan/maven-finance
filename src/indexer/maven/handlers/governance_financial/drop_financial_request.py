...
async def drop_financial_request(
    ctx: HandlerContext,
    drop_financial_request: TezosTransaction[DropFinancialRequestParameter, GovernanceFinancialStorage],
) -> None:

    try:    
        # Get operation info
        financial_address   = drop_financial_request.data.target_address
        request_id          = int(drop_financial_request.parameter.root)
        request_storage     = drop_financial_request.storage.financialRequestLedger[drop_financial_request.parameter.root]
